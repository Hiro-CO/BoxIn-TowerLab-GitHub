...
namespace Playmove
{
    ...
    public class PYBundleManager : MonoBehaviour
    {
        ...
        public string ExpansionName { get { return PlaytableWin32.Instance.ExpansionName; } set { PlaytableWin32.Instance.ExpansionName = value; } }
        public string Language { get { return PlaytableWin32.Instance.Language; } set { PlaytableWin32.Instance.Language = value; } }
            ...
            for (int x = 0; x < paths.Count; x++)
            {
                if (paths[x].Contains("string") && paths[x].Contains(".json"))
                {
                    using (StreamReader reader = new StreamReader(paths[x]))
                    {
                        ...
                        paths.RemoveAt(x);
                        foreach (LocalizationData data in jsonObject.Dados)
                        {
