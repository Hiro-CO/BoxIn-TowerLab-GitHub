...
namespace Playmove
{
    ...
    public abstract class PYComponentBundle : MonoBehaviour, IPYComponentBundle
    {
        ...
        {
            if (Application.isPlaying && UpdateData.UpdateFromBundle)
                InitializeComponent();
        }
        ...
        {
            if (PYBundleManager.Instance == null) return;

            if (PYBundleManager.Instance.State == PYBundleManagerState.Ready)
                UpdateComponent();
            else
                PYBundleManager.Instance.onLoadCompleted.AddListener(BundleLoadCompletedCallback);
        }
